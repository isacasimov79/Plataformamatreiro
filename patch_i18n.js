...
const files = [
    { path: './src/i18n/locales/pt-BR.json', data: pt },
    { path: './src/i18n/locales/en.json', data: en },
    { path: './src/i18n/locales/es.json', data: es }
];

files.forEach(file => {
    let content = JSON.parse(fs.readFileSync(file.path, 'utf8'));
    content.enhancedAnalytics = file.data.enhancedAnalytics;
