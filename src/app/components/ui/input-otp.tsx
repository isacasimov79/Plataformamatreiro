...
function InputOTPSlot({ index, className, ...props }: React.ComponentProps<"div"> & { index: number }) {
  ...
  return (
    <div
      ...
      className={cn(
        "relative flex h-9 w-9 items-center justify-center border-y border-r text-sm transition-all outline-none",
        "first:rounded-l-md first:border-l last:rounded-r-md",
        "bg-white dark:bg-[#1a1a2e]",
        "border-[#e5e7eb] dark:border-[#3a3a5e]",
        "data-[active=true]:z-10 data-[active=true]:ring-[3px]",
        "data-[active=true]:border-[#834a8b] data-[active=true]:ring-[#834a8b]/50",
    ...
    >
      ...
      {hasFakeCaret && (
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          <div className="animate-caret-blink h-4 w-px bg-[#242545] dark:bg-white duration-1000" />
        </div>
      )}
    </div>
  );
